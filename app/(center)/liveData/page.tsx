...
import {
	Button, Spacer, Spinner, CardBody, Card, Input, Image,
	Table, TableHeader, TableColumn, TableBody, TableRow, TableCell, CardHeader, Textarea,
} from "@nextui-org/react";
import { useEffect, useRef, useState } from "react";
import dayjs from "dayjs";
import { applyLicense, cancelLicense, getPersonalLiveInfo, getPersonalLicense, updateLiveInfo } from "@/api/live";
...
const LiveInfoBox = (
	...
	const handleLiveInfoUpdate = async () => {
		if (!coverUploadResult.filePath || !title || title.length <= 0 || introduction.length > 50) {
...
export default function Page() {
	...
	return (
		<div className="w-full flex flex-col gap-8">
			<Card shadow="sm">
				<CardBody>
					<p>If you want to publish by livestream, please use the token template</p> <Spacer />
					<p>Template: {`${liveHost}/Your License Key?secret=Your License Secret`}</p> <Spacer />
					...
					<p>Like: {`${liveHost}/123?secret=123`}</p><Spacer />
				</CardBody>
			</Card>
			<Table
			...
			>
				...
				<TableBody
					items={liveLicense}
					isLoading={isLoading}
					loadingContent={<Spinner color="warning" className="mt-12" />}
					emptyContent={
						<Button color="primary" onPress={handleApply} >Apply License</Button>
					}
				>
					{
						(item) => (
							<TableRow key="0">
								...
								<TableCell>
									{
										(!item?.superviseLicense || item?.superviseLicense.status === AuditStatusType.UNAPPROVED) ?
											<Button color="primary" onPress={handleApply} >Apply</Button>
											:
											<Button color="primary" onPress={handleCanel}>Cancel</Button>
									}
								</TableCell>
							</TableRow>
						)
			...
			{
				liveLicense.length > 0 && liveLicense[0].live && <LiveInfoBox liveId={liveLicense[0].live?.id} />
			}
		</div>
	);
}
