...
const VideoSearchContent = () => {
  ...
  const fetchData = async () => {
    ...
    const { result } = await searchVideoList({
      ...
      categoryId: categorySelected !== '-1' ? Number.parseInt(categorySelected) : undefined,
      sortBy: sortSelected,
      pageNo: pageNo,
  ...
  return (
    <div className="flex flex-col gap-8">
        ...
        <div className="flex gap-4">
          {
            categoryList.map(item =>
              <Chip radius="sm" color="primary" key={item.id}
                onClick={() => {
                  setPageNo(1)
      ...
      {
        isLoading ?
          <Spinner color='warning' className="mt-32"
            classNames={{
              wrapper: 'w-20 h-20'
            }}
          /> :
          searchParams.get('keyword') !== null && pageParams.total > 0 ?
            <>
    ...
    <Tabs className="w-full"
      ...
      variant="underlined">
      {
        tabList.map((item, index) =>
          <Tab key={index} title={item.title} className="w-full">
            <div className="px-16">
              {item.content}
            </div>
          </Tab>
